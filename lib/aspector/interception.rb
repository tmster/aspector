module Aspector
  class Interception
    ...
    def logger
      ...
      @logger.level = @aspect.class.logger.level
      @logger
    end
    ...
    def recreate_method_with_advices method, before_advices, after_advices, around_advice, is_outermost = false
      ...
      logger.log Logging::DEBUG, 'generate-code', method, code
...
% if logger.visible?(Logging::TRACE)
      logger.log <%= Logging::TRACE %>, '<%= method %>', 'enter-generated-method'
% end

      if aspect.disabled?
% if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'exit--generated-method'
% end
...
%   if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'before-invoke-advice', '<%= advice.name %>'
% end
...
%   end
% if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'after--invoke-advice', '<%= advice.name %>'
% end
%   if advice.options[:skip_if_false]
        unless result
% if logger.visible?(Logging::TRACE)
          logger.log <%= Logging::TRACE %>, '<%= method %>', 'exit-method-due-to-before-filter', '<%= advice.name %>'
% end
          return
        end
%   end
...
% if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'before-invoke-advice', '<%= around_advice.name %>'
% end
...
% if logger.visible?(Logging::TRACE)
        proxy = lambda do |*args, &block|
          logger.log <%= Logging::TRACE %>, '<%= method %>', 'before-invoke-proxy'
          res = wrapped_method.bind(self).call *args, &block
          logger.log <%= Logging::TRACE %>, '<%= method %>', 'after--invoke-proxy'
          res
        end
...
%   end
% if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'after--invoke-advice', '<%= around_advice.name %>'
% end
...
% if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'before-wrapped-method'
% end
        result = orig_method.bind(self).call *args, &block
% if logger.visible?(Logging::TRACE)
        logger.log <%= Logging::TRACE %>, '<%= method %>', 'after--wrapped-method'
% end
